...
const ItemList = ({items, onClickFun, string}) => {

    const idSet = new Set();
    items.forEach(item => {
    const id = item.card.info.id;
    if (idSet.has(id)) {
        console.warn("Duplicate key found:", id);
    ...
    return <div>
        <ul>
            {items.map ( (item, index) => 
                <div key={`${item.card.info.id}-${index}`} className="m-3 p-2 border-b-2 border-gray-300 flex items-center justify-between gap-12">
                    <div className="w-9/12">
                        ...
                        <span className="font-medium"> ₹{item.card.info.defaultPrice ? item.card.info.defaultPrice / 100 : item.card.info.price}</span>
                    ...
                    <div className="w-3/12 my-3">
                        ...
                        <div className="absolute translate-x-6.5 -translate-y-5.5">
                            <button className="bg-white text-green-600 rounded-xl py-2 px-8 text-lg font-bold cursor-pointer" onClick={() => onClickFun(item)}>{string}</button>
                        </div>
